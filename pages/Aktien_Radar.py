"""
╔══════════════════════════════════════════════════════════╗
║   MSCI ACWI GLOBAL MULTIPLEX QUANT RADAR (V7.0 - LAIEN)  ║
║   Premium Report-Engine mit Kennzahlen & Erklärungen     ║
╚══════════════════════════════════════════════════════════╝
"""
...
st.markdown("""
    <div style="background: linear-gradient(135deg, #1e1b4b 0%, #0f172a 100%); 
        ...
        <h1 style="color: white; margin: 0 0 12px 0;">🌍 MSCI ACWI Global Insights — V7.0</h1>
...
def fetch_social_volume_via_ki(api_key, ticker):
    ...
    try:
        ...
        return {"volume_growth_pct": 0, "reddit_sentiment": "Neutral", "hot_topic": "Unauffälliges, stabiles Handelsvolumen in Forennetzwerken."}
...
if scan_btn:
    ...
    for c_idx, chunk in enumerate(chunks):
        status_text.markdown(f"📡 Mathematische Filteranalyse läuft... Segment {c_idx+1}/{len(chunks)} ({len(chunk)} Aktien)...")
        try:
            raw_data = yf.download(chunk, period="3mo", group_by='ticker', progress=False) # Erweitert auf 3 Monate für stabilere Indikatoren
                ...
                try:
                    ...
                    if math_points > 0:
                        math_results.append({
                            ...
                            "math_points": math_points, "patterns": signals, "math_explanations": explanations, "df": df
                        })
    ...
    if not math_results:
        st.error("⚠️ Keine mathematischen Ausbruchskandidaten gefunden. Senke eventuell die Hürde.")
        st.stop()
        
    # Top 4 isolieren, um Yahoo-Fundamentaldaten stabil und schnell zu ziehen
    math_results = sorted(math_results, key=lambda x: x["math_points"], reverse=True)[:4]
    
    st.success(f"💎 {len(math_results)} Top-Kandidaten isoliert. Generiere Fundamentalanalyse & KI-Lagebild...")
    
    final_results = []
    
    for idx, stock in enumerate(math_results):
        # 1. Social Deep Dive per KI
        ...
        # 2. Echte Fundamentaldaten über yfinance holen
    ...
    for s in output:
        ...
        # Hochgradig übersichtliches & sauberes Candlestick-Diagramm
        df = s["df"]
        ...
        # Laien-Erklärung für das Diagramm direkt darunter platzieren
        st.markdown("""
