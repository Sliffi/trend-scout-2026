"""
╔══════════════════════════════════════════════════════════╗
║   MSCI ACWI GLOBAL MULTIPLEX QUANT RADAR (V6.1 - FIXED)  ║
║   Fehlertolerante KI-Validierung & Multi-Chunk Engine    ║
╚══════════════════════════════════════════════════════════╝
"""
...
st.markdown("""
    <div style="background: linear-gradient(135deg, #1e1b4b 0%, #0f172a 100%); 
        ...
        <h1 style="color: white; margin: 0 0 12px 0;">🌍 MSCI ACWI World Quant Scanner — Fixed V6.1</h1>
...
if scan_btn:
    ...
    for idx, stock in enumerate(math_results):
        ...
        # 💥 DIE ENTSCHEIDENDE RETTUNG (3-MAL HINTERFRAGT):
        # Wir zwingen den Typ zu int, falls Gemini Text ("Keins", "N/A", "Null") liefert!
        try:
            growth = int(social_data.get("volume_growth_pct", 0))
