"""
╔══════════════════════════════════════════════════════════╗
║   MSCI ACWI GLOBAL MULTIPLEX QUANT RADAR (V6.0 - MEGA)   ║
║   Unkaputtbare Chunk-Engine für volle 200+ Aktien-Listen ║
╚══════════════════════════════════════════════════════════╝
"""
...
# ── Page-Config ─────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="MSCI ACWI Global Radar Mega",
    page_icon="🌍",
    layout="wide",
)
...
st.markdown("""
    <div style="background: linear-gradient(135deg, #1e1b4b 0%, #0f172a 100%); 
        ...
        <h1 style="color: white; margin: 0 0 12px 0;">🌍 MSCI ACWI World Quant Scanner — Mega V6.0</h1>
        <p style="color: #93c5fd; margin: 0; font-size: 1.05rem;">
            Fehlertolerante Multi-Chunk Engine für das volle 200er Universum. Stand: Juni 2026.
        </p>
    </div>
""", unsafe_allow_html=True)

market_type = st.selectbox(
    "Wähle das MSCI ACWI Universum (Volle 200er Listen):",
    [
...
# ── DIE VOLLSTÄNDIGEN, BEREINIGTEN 200er LISTEN ──────────────────────────────
...
scan_btn = st.button(f"🚀 Globalen MEGA-Multiplex-Scan starten (Volle {len(MARKET_LISTS[market_type])} Aktien)", use_container_width=True)
...
def fetch_social_volume_via_ki(api_key, ticker):
    ...
    try:
        ...
        return {"volume_growth_pct": 0, "reddit_sentiment": "Neutral", "hot_topic": "Stabile Akkumulation."}
...
if scan_btn:
    ...
    # CHUNKING ENGINE: Wir teilen die 200 Aktien in 10er Pakete auf
    chunk_size = 10
    chunks = [full_tickers[i:i + chunk_size] for i in range(0, len(full_tickers), chunk_size)]
    
    progress_bar = st.progress(0)
    status_text = st.empty()
    
    for c_idx, chunk in enumerate(chunks):
        status_text.markdown(f"📡 Verarbeite Marktsegment {c_idx+1} von {len(chunks)} ({len(chunk)} Aktien)...")
        
        try:
            # Schneller Batch-Download für dieses kleine Segment
            raw_data = yf.download(chunk, period="1mo", group_by='ticker', progress=False)
            
            for ticker in chunk:
                try:
                    # Robustes Heraustrennen aus dem Multi-Index des Chunks
                    ...
                    change_pct = ((float(df['Close'].iloc[-1]) - float(df['Close'].iloc[-2])) / float(df['Close'].iloc[-2])) * 100
                    if abs(change_pct) > 0.8:
                        math_points += 15
                        
                    math_results.append({
                        "ticker": ticker, "price": float(df['Close'].iloc[-1]), "change": change_pct,
                        "math_points": math_points, "patterns": signals, "df": df
                    })
                except Exception:
                    continue  # Fängt Fehler innerhalb einer Aktie ab
                    
        except Exception:
            continue  # Fängt Serverfehler des gesamten Chunks ab und macht unbeeindruckt weiter!
            
        progress_bar.progress(int((c_idx + 1) / len(chunks) * 100))
        
    progress_bar.empty()
    status_text.empty()
    
    if not math_results:
        st.error("⚠️ Keine globalen Daten empfangen. Bitte starte den Scan erneut.")
        st.stop()
        
    # Top 6 aus allen erfolgreichen Scans filtern
    math_results = sorted(math_results, key=lambda x: x["math_points"], reverse=True)[:6]
    
    st.success(f"🎯 Phase 2: {len(math_results)} Top-Kandidaten aus dem 200er Universum isoliert. Starte OSINT Deep Dive...")
    ...
    for idx, stock in enumerate(math_results):
        ...
        if growth >= 20: stock["patterns"].append(f"🔥 Social-Spike (+{growth}%)")
            
        stock["probability"] = final_probability
        stock["social_sentiment"] = social_data.get("reddit_sentiment", "Neutral")
        stock["social_topic"] = social_data.get("hot_topic", "Keine Ausreißer.")
        ...
        st.markdown(f"""
            <div class="stock-box">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                    <div><span style="font-size: 1.6rem; font-weight: 800; color: #60a5fa;">📈 {s['ticker']}</span></div>
